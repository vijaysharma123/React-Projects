...
function App() {
  ...
  const fetchWeather = async (searchVal) => {
    try {
      let uri = `https://api.openweathermap.org/data/2.5/weather?q=${searchVal}&units=metric&appid=b1307ae5386855405cb82858030eaece`
      await axios.get(uri)
      .then((res) => {
  ...
  const fetchDailyData = async (lat, lon) => {
    try {
      let uri = `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&exclude=hourly,minutely&units=metric&appid=b1307ae5386855405cb82858030eaece`;
      await axios.get(uri)
      .then((res) => {
        console.log(res.data);
